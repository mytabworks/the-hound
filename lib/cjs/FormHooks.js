...
export const useGetValue = (name, isArray = false) => {
  const {
    fields
  } = useFormField();
  return useMemo(() => {
    return isArray ? name in fields ? transformFieldsToJSON(fields[name]) : [] : findOrCreateField(fields[name]).value;
...
export const useForm = (defaultSchema = {}) => {
  ...
  const formState = useCallback(name => typeof name === 'string' ? findOrCreateField(fields[name]) : immutableFields(fields), [fields]);
  ...
  const getValue = useCallback((name, isArray = false) => typeof name === 'string' ? isArray ? name in fields ? transformFieldsToJSON(fields[name]) : [] : findOrCreateField(fields[name]).value : transformFieldsToJSON(fields), [fields]);
