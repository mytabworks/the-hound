...
export const useGetValue = name => {
  const {
    fields
  } = useFormField();
  return useMemo(() => {
    return Array.isArray(fields[name]) ? name in fields ? transformFieldsToJSON(fields[name]) : [] : findOrCreateField(fields[name]).value;
...
export const useForm = (defaultSchema = {}) => {
  ...
  const formState = useCallback(name => typeof name === 'string' ? Array.isArray(fields[name]) ? name in fields ? immutableFieldArray(fields[name]) : [] : findOrCreateField(fields[name]) : immutableFields(fields), [fields]);
  ...
  const getValue = useCallback(name => typeof name === 'string' ? Array.isArray(fields[name]) ? name in fields ? transformFieldsToJSON(fields[name]) : [] : findOrCreateField(fields[name]).value : transformFieldsToJSON(fields), [fields]);
  const getFieldArray = useCallback(name => {
